...
import type { FoodRiskWorker } from "./food-risk-worker";
...
export class FoodRiskDispatcher implements OnApplicationBootstrap, OnApplicationShutdown {
  private readonly logger = new Logger(FoodRiskDispatcher.name);
  private timer: NodeJS.Timeout | null = null;
  private active: Promise<boolean> | null = null;

  constructor(
    private readonly database: PrismaClient,
    private readonly worker: FoodRiskWorker,
  ...
  private schedule(): void {
    ...
    const run = this.drainOnce().catch(() => {
      this.logger.error("Food scan drain failed; the lease remains recoverable");
      return false;
    });
    this.active = run;
    void run.finally(() => {
